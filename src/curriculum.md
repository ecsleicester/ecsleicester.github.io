---
layout: page.njk
title: "Curriculum"
description: "What pupils at Emmanuel Christian School are taught, from age 4 to GCSE, and the Christian framework that shapes it."
permalink: /curriculum/
crumb: "<a href='/'>Home</a>  ›  Curriculum and Learning"
heading: "What your child would learn here"
standfirst: "A full curriculum, the same GCSEs as anywhere else, taught by people who believe Christ holds the whole of it together."
draft: "Subject lists, class age ranges and staff names on this page are placeholders and need checking against the 2026/27 timetable before publication."
contents:
  - id: aims
    text: "What we are trying to do"
  - id: framework
    text: "Christ in All Things"
  - id: structure
    text: "How we are organised"
  - id: lower
    text: "Lower School"
  - id: middle
    text: "Middle School"
  - id: upper
    text: "Upper School and GCSE"
  - id: assessment
    text: "How we assess"
  - id: send
    text: "Additional needs"
  - id: rshe
    text: "RSHE"
  - id: careers
    text: "Careers guidance"
  - id: beyond
    text: "Beyond the timetable"
  - id: documents
    text: "Policies"
nextprev: |
  <div class="nextprev">
  <a class="np" href="/results/"><span>Next</span><b>Examination results →</b></a>
  <a class="np" href="/admissions/"><span>Admissions</span><b>Come and visit →</b></a>
  </div>
eleventyNavigation:
  key: curriculum
---

## What we are trying to do {#aims}

<p class="lead">We want every child who leaves Emmanuel at sixteen to read well, think clearly, work hard at things they find difficult, and understand the world as God’s world. The qualifications matter, and we take them seriously. They are not the point.</p>

Pupils here are taught the same subjects and sit the same GCSEs as pupils anywhere else in Leicester. What differs is the account we give of what they are learning. We do not teach a separate Christian version of science or history. We teach science and history, taught well, by people who believe the order being studied was put there.

<div class="pull">He is before all things, and in him all things hold together.
<span class="verse">Colossians 1:17</span></div>

## Christ in All Things {#framework}

*Christ in All Things* is the framework that shapes our curriculum across the whole school. It runs on a simple discipline: a connection between a subject and the Christian faith is only worth making if it would survive the scrutiny of a thoughtful sixteen-year-old. Forced analogies do more harm than good, and pupils see through them immediately.

In practice this looks like six threads running through the subjects — creation and order, image and dignity, brokenness and repair, stewardship, truth and honesty, and hope. A geography lesson on flooding touches stewardship without being interrupted by it. A design lesson on failure touches repair. The subject stays the subject.

The framework is being introduced in phases across the school. Some subjects are fully rebuilt around it, others carry it more lightly at this stage, and we would rather tell you that plainly than claim a finished job.

## How the school is organised {#structure}

We teach in seven mixed-age classes. A child joining in Hazel at four can stay through to Ash and leave with GCSEs at sixteen, taught along the way by nearly every member of staff.

Mixed-age teaching is a deliberate choice, not only a consequence of our size. Children work at the level they are actually at rather than the level their birthday implies, older pupils learn by explaining, and a child who needs another year on something can have it without moving class or losing face.

<table>
<caption>Classes and phases</caption>
<thead><tr><th style="width:22%">Class</th><th style="width:20%">Ages</th><th style="width:24%">Phase</th><th>Taught by</th></tr></thead>
<tbody>
<tr><td class="num">Hazel</td><td>4–6 <span class="todo">confirm</span></td><td>Lower School</td><td>Class teacher, with specialist input</td></tr>
<tr><td class="num">Oak</td><td>6–8 <span class="todo">confirm</span></td><td>Lower School</td><td>Class teacher, with specialist input</td></tr>
<tr><td class="num">Willow</td><td>8–9 <span class="todo">confirm</span></td><td>Lower School</td><td>Class teacher, with specialist input</td></tr>
<tr><td class="num">Cedar</td><td>9–11 <span class="todo">confirm</span></td><td>Middle School</td><td>Class teacher moving to subject teachers</td></tr>
<tr><td class="num">Maple</td><td>11–13 <span class="todo">confirm</span></td><td>Middle School</td><td>Subject teachers</td></tr>
<tr><td class="num">Birch</td><td>13–14 <span class="todo">confirm</span></td><td>Upper School</td><td>Subject teachers</td></tr>
<tr><td class="num">Ash</td><td>14–16 <span class="todo">confirm</span></td><td>Upper School</td><td>Subject teachers</td></tr>
</tbody>
</table>

## Lower School {#lower}

The first years are about securing reading, writing and number, because everything after depends on them. Children who read fluently and early can access the whole curriculum; children who do not spend the rest of their schooling catching up. We would rather spend the time now.

Alongside that, pupils are taught science, history, geography, art, design and technology, music, physical education and Bible. Much of it is organised into topics so that a child sees how subjects speak to one another, but the subject knowledge underneath is planned and sequenced deliberately.

Mathematics follows a mastery approach built on the White Rose sequence, adapted for mixed-age classes. Children move on when they have understood, not when the calendar says so.

## Middle School {#middle}

Cedar and Maple are where the shift happens: from one teacher who knows a child in every subject, to several teachers who each know them in one. We do this gradually, and the class keeps a form teacher who holds the whole picture.

Pupils are taught English, mathematics, science, history, geography, religious education, modern foreign languages, art, design and technology, music, drama, computing, physical education and PSHE. World religions are taught from Year 7 upwards: our pupils should be able to give a fair account of what a Muslim or a Hindu actually believes, not a caricature of it.

<span class="todo">Confirm subject list against the 2026/27 timetable</span>

## Upper School and GCSE {#upper}

Birch and Ash work towards GCSE. Courses are examined with AQA and Eduqas. Because our cohorts are small, pupils are taught in groups where nobody can sit quietly at the back and nobody’s misunderstanding goes unnoticed for a fortnight.

<table>
<caption>GCSE courses offered</caption>
<thead><tr><th style="width:44%">Subject</th><th style="width:26%">Board</th><th>Notes</th></tr></thead>
<tbody>
<tr><td>English Language and English Literature</td><td><span class="todo">confirm</span></td><td>Both taken by all pupils</td></tr>
<tr><td>Mathematics</td><td>AQA</td><td>Foundation or Higher</td></tr>
<tr><td>Combined Science</td><td>AQA</td><td><span class="todo">confirm separate sciences</span></td></tr>
<tr><td>Business</td><td>AQA (8132)</td><td></td></tr>
<tr><td>Religious Studies</td><td><span class="todo">confirm</span></td><td></td></tr>
<tr><td>Design and Technology</td><td><span class="todo">confirm</span></td><td></td></tr>
<tr><td>Physical Education</td><td><span class="todo">confirm</span></td><td></td></tr>
<tr><td><span class="todo">Remaining options to be listed</span></td><td></td><td></td></tr>
</tbody>
</table>

[See our most recent examination results →](/results/)

## How we assess {#assessment}

Assessment exists to tell a teacher what to do next. Pupils are assessed continuously in lessons, with formal assessment points across the year recorded on our management information system. From September 2026 that system is Go4Schools, which also gives parents a live view of their child’s work rather than waiting for a report.

Parents receive a written report each year and are invited to consultation evenings each term. If something is going wrong, we will not wait for the report to tell you.

## Pupils with additional needs {#send}

Every pupil with an identified need has a Passport: a short working document that says what helps, what hinders, and what every adult teaching that child should do. It is written with the pupil and their parents, and reviewed termly.

Our SENDCo is Libby Harris. Small classes mean needs are usually noticed early, and adjustments are made without fuss. We also make provision for pupils with English as an additional language and for pupils with education, health and care plans.

[Read more about SEND and support →](/send/)

## Relationships, sex and health education {#rshe}

Our programme is called *Growing in Christ*. It is taught across seven strands from the youngest class upwards, at a level appropriate to each age, and is rooted in the biblical account of what people are for. It meets the statutory requirements in full.

Parents have the right to request that their child is withdrawn from sex education, and we will always talk that through with you rather than process a form.

[Read the full RSHE programme and policy →](/rshe/)

## Careers guidance {#careers}

Pupils in the secondary years receive impartial careers guidance covering the full range of options open to them: sixth form, college, apprenticeships and employment. It is impartial in the proper sense — we do not steer pupils towards a particular path, including a Christian one.

<span class="todo">Describe the careers programme: who delivers it, when, and what pupils receive</span>

## Beyond the timetable {#beyond}

Trips, sport, music, and the wider life of the school are part of the curriculum, not decoration around it.

<span class="todo">Add the clubs, trips and enrichment actually running in 2026/27</span>

<div class="note">
<p><b>Want to see it rather than read about it?</b></p>
<p>The best way to understand how we teach is to watch a lesson. Visits happen on ordinary working days, with the Head Teacher, and take about an hour.</p>
<p style="margin-top:16px"><a class="btn btn-gold" href="/admissions/">Book a visit</a></p>
</div>

## Policies and documents {#documents}

<div class="docs">
<a class="doc" href="/policies/"><div><b>Curriculum policy</b><span>PDF · reviewed <span class="todo">date</span></span></div></a>
<a class="doc" href="#"><div><b>Christ in All Things framework</b><span>PDF · <span class="todo">date</span></span></div></a>
<a class="doc" href="#"><div><b>RSHE policy</b><span>PDF · <span class="todo">date</span></span></div></a>
<a class="doc" href="#"><div><b>SEND policy</b><span>PDF · <span class="todo">date</span></span></div></a>
</div>

<hr class="rule"/>

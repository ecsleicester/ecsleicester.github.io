---
layout: page.njk
title: "Illness and absence"
description: "Reporting absence, when to keep your child at home, medication and first aid at Emmanuel Christian School."
permalink: /absence/
crumb: "<a href='/'>Home</a>  ›  <a href='/parents/'>Parents</a>  ›  Illness and absence"
heading: "Illness and absence"
standfirst: "How to tell us, when to keep them at home, and what happens about medication."
draft: "Exclusion periods must be checked against current UKHSA guidance, and the medication procedure against the school policy."
contents:
  - id: report
    text: "Reporting an absence"
  - id: leave
    text: "Requesting leave"
  - id: whenkeep
    text: "When to keep them at home"
  - id: medication
    text: "Medication"
  - id: firstaid
    text: "First aid"
  - id: allergies
    text: "Allergies"
  - id: lateness
    text: "Lateness"
  - id: attendance
    text: "Attendance"
nextprev: |
  <div class="nextprev">
  <a class="np" href="/uniform/"><span>Back</span><b>← Uniform and PE kit</b></a>
  <a class="np" href="/policies/"><span>Next</span><b>Policies →</b></a>
  </div>
---

## Reporting an absence {#report}

<p class="lead">The quickest way is the Parent Hub. It reaches the office and senior staff immediately and sends you a confirmation email, so you have a record that it arrived.</p>

<div class="note">
<p><b>Report an absence in the Parent Hub</b></p>
<p style="margin:0"><a class="btn btn-gold" href="https://script.google.com/macros/s/AKfycbyFia18Og2msKpA2PTSui3ltNBhh9Cn1G652KY4EbvmWkS4EnAaeXTRzV7mShe6DgxCyg/exec" target="_blank" rel="noopener">Report an absence &rarr;</a></p>
<p style="margin-top:12px">Sign in with your family passcode. One passcode covers all your children.</p>
</div>

If you would rather ring, or you cannot get into the Hub, telephone the school before 9.00am on each day your child is absent.

- Telephone: [0116 222 0792](tel:01162220792)
- Email: [office@ecsleicester.org.uk](mailto:office@ecsleicester.org.uk)

If we have not heard from you we will contact you, because we have to know that every child is safe.

## Requesting leave {#leave}

Leave in term time — a medical appointment, a family occasion, anything planned — is requested through the Parent Hub rather than by letter. The request goes to the Head Teacher and you are emailed once a decision has been made.

[Our position on holidays in term time →](/termdates/)

## When to keep your child at home {#whenkeep}

The general rule is simple: if your child is well enough to take part in the school day, send them. Children who are miserable but not infectious usually do better in school than at home.

The exclusion periods below follow national guidance on infections in schools. 

<table>
<caption>Common illnesses</caption>
<thead><tr><th style="width:44%">Illness</th><th>Keep at home for</th></tr></thead>
<tbody>
<tr><td class="num">Vomiting or diarrhoea</td><td>48 hours after the last episode</td></tr>
<tr><td class="num">Chickenpox</td><td>Until all the spots have crusted over, usually about five days after they appear</td></tr>
<tr><td class="num">High temperature</td><td>Until it has settled and they feel well enough</td></tr>
<tr><td class="num">Impetigo</td><td>Until lesions are crusted or 48 hours after starting treatment</td></tr>
<tr><td class="num">Conjunctivitis</td><td>No need to stay at home</td></tr>
<tr><td class="num">Head lice</td><td>No need to stay at home. Please treat and tell us.</td></tr>
<tr><td class="num">Threadworms</td><td>No need to stay at home once treatment has started</td></tr>
</tbody>
</table>

If you are unsure, ring us. We would rather have the conversation than have you guess.

## Medication in school {#medication}

We can administer prescribed medication where a form has been completed and the medicine is in its original labelled packaging. Nothing is given without written consent.

A medication form must be completed in person at school before we can give your child anything, including inhalers and adrenaline auto-injectors. If your child has a long-term condition, talk to us so that we can agree arrangements properly rather than form by form.

## First aid {#firstaid}

Trained first aiders are on site throughout the school day. If your child is hurt beyond a minor scrape you will be told the same day, not at the end of the week.

Our trained first aiders are Sarah Seeds, George Sutton, Mia Seeds and Ruth Allen.

<div class="docs">
<a class="doc" href="/assets/policies/first-aid-policy.pdf"><div><b>First aid policy</b><span>PDF &middot; reviewed August 2024</span></div></a>
<a class="doc" href="/assets/policies/policy-for-supporting-pupils-at-school-with-medical-conditions.pdf"><div><b>Supporting pupils with medical conditions</b><span>PDF &middot; reviewed September 2024</span></div></a>
</div>

## Allergies {#allergies}

**We are a nut-free school.** Please do not send anything containing nuts in a packed lunch or a snack, including chocolate spreads and cereal bars that contain them.

Tell us in writing about any allergy, however minor it seems, and update us when things change.

## Lateness {#lateness}

Registration closes at 9.00am. Children arriving after that should come to the office so that we know they are in the building.

## Attendance {#attendance}

Attendance is not a statistic we chase for its own sake. In a small mixed-age class, a child who misses a fortnight is genuinely hard to catch up, and we would rather work with you early than write letters later. If your child is finding it hard to come in, tell us. That is a problem we can help with.

<div class="docs">
<a class="doc" href="/policies/"><div><b>Attendance policy</b><span>PDF · reviewed August 2025</span></div></a>
<a class="doc" href="/termdates/"><div><b>Term time holidays</b><span>Our position</span></div></a>
</div>

<hr class="rule"/>
